...
import misaka.nemesiss.com.findlostthings.Adapter.LostThingDetailAdapter;
...
public class SearchActivity extends FindLostThingsActivity {
    ...
    @BindView(R.id.IsGivenState)
    RadioGroup IsGivenState;
    ...
    private LostThingDetailAdapter thingsDetailedAdapter = new LostThingDetailAdapter(SearchActivity.this, thingsDetails);
...
//        dt.getName();
    ...
    private void InitComponents() {
        ...
        IsGivenState.check(R.id.IsGiven0);
    }
        ...
        switch (item.getItemId()) {
            case R.id.PublishTickBtn: {
                if (!ValidateAllFields()) {
                    SearchLostThingsInfo request = BuildupSearchThingRequest();
                    Intent intent = new Intent(SearchActivity.this, SearchResultActivity.class);
                    intent.putExtra("SearchLostThingsInfo", request);
    ...
    private SearchLostThingsInfo BuildupSearchThingRequest() {
        ...
        searchLostThingsInfo.setIsAdvancedSort(0);
        searchLostThingsInfo.setAdvancedSortText(null);

        searchLostThingsInfo.setSortType(0); // 按发布时间排序

        switch (IsGivenState.getCheckedRadioButtonId()) {
            case R.id.IsGiven0 : {
                searchLostThingsInfo.setItemStatus(1); // 搜索未完成
                break;
            }
            case R.id.IsGiven1 : {
                searchLostThingsInfo.setItemStatus(2); // 搜索已完成
                break;
            }
            case R.id.IsGivenAll : {
                searchLostThingsInfo.setItemStatus(3); // 搜索全部
                break;
            }
        }
